...
[HarmonyPatch(typeof(MainMenu))]
    ...
    {
        public static string[] BlacklistedItems = Array.Empty<string>();
        public static string[] WhitelistedItems = Array.Empty<string>();
        ...
        {
            Debug.Log("Adding MU Cables");
            Main.modEntry.Logger.Log("Using Settings of : \n PatchAllMode: "+PatchAllMode+"\n AltMUPos: "+ AltMUPos +"\n TenderMU: "+ TenderMU+"\n Be2MU: "+Be2MU);
            if (HasPatched)
            {
                Main.modEntry.Logger.Error("Game has been loaded before; Aborting!");
                return;
            }
            HasPatched = true;
            Main.modEntry.OnGUI = null;
            if(Main.MPEnabled)Main.BlockMP();
            ...
            foreach (TrainCarLivery livery in Globals.G.Types.Liveries)
            {
                ...
                if (IsBlacklisted(livery.prefab.name))
                {
                    Main.modEntry.Logger.Log($"Skipping {livery.prefab.name}: Found in blacklist.");
                    continue;
                }
                
                if (!PatchAllMode && IsWhitelisted(livery.prefab.name)) MuLiveries.Add(livery);
        ...
        private static void ControllerFix()
        {
            foreach (var item in Globals.G.Items.items)
            {
                ...
                {
                    Main.modEntry.Logger.Log("Fixing Item Gadget of: "+item.name);
        ...
        private static void AddMuTo(TrainCarLivery carToMU, TrainCarLivery de6)
        {
            ...
            Main.modEntry.Logger.Log("MU-ing Car Type of: "+carToMU.prefab.name);
            ...
            try
            {
                ...
                {
                    Main.modEntry.Logger.Log("Car Type of: " + carToMU.prefab.name +
                                             " is incompatible or already has MU.");
                ...
                if (carToMU.prefab.transform.Find("[sim]") == null)
                {
                    ...
                    carToMU.prefab.gameObject.GetOrAddComponent<SimController>().connectionsDefinition =
                        sim.GetOrAddComponent<SimConnectionDefinition>();
                }
                ...
                carToMU.prefab.gameObject.GetComponent<SimController>().controlsOverrider =
                    controles.GetOrAddComponent<BaseControlsOverrider>();
                ...
                if (CarTypes.IsTender(carToMU)) muApos = new Vector3(-muApos.x, muApos.y + 0.16f, muApos.z + 0.1f);
                    ...
                    muApos = new Vector3(muApos.x + AltMuXOffset, muApos.y - AltMuYOffset, muApos.z - MuZOffset);
                ...
                {
                    carToMuBuffs[0].Find("hoses").Find("CouplingHoseRig").position =
                        new Vector3(muApos.x - AltHoseXOffset, muApos.y + AltHoseYOffset, muApos.z);
                }



                //setup object
                carToMuBuffs[0].Find("hoses").GetComponent<CouplingHoseDelayedEnable>().childrenToEnable =
                    ExtendHoseChildren(
                        carToMuBuffs[0].Find("hoses").GetComponent<CouplingHoseDelayedEnable>().childrenToEnable,
                        newMUA.gameObject);
                ...
                if (carToMuBuffs.Count > 1)
                {
                    ...
                    if (AltMUPos)
                        muBpos = new Vector3(muBpos.x - AltMuXOffset, muBpos.y - AltMuYOffset, muBpos.z + MuZOffset);
                    ...
                    if (AltMUPos)
                    {
                        carToMuBuffs[1].Find("hoses").Find("CouplingHoseRig").position =
                            new Vector3(muBpos.x + AltHoseXOffset, muBpos.y + AltHoseYOffset, muBpos.z);
                    }

                    //setup object
                    carToMuBuffs[1].Find("hoses").GetComponent<CouplingHoseDelayedEnable>().childrenToEnable =
                        ExtendHoseChildren(
                            carToMuBuffs[1].Find("hoses").GetComponent<CouplingHoseDelayedEnable>().childrenToEnable,
                            newMUB.gameObject);
            ...
            catch (Exception e)
            {
                ...
                Debug.LogWarning("Failed to MU "+carToMU.prefab.name+"!\n This may cause bugs! Use 'blacklist.txt' in the mod folder to ignore this car!");
            }
        ...
        public static void SetPrivateField<TInstance>(TInstance instance, string fieldName, object value)
        {
            ...
            if (field == null)
            {
                Main.modEntry.Logger.Error($"Could not find a property, private field, or property backing field named '{fieldName}' on instance of type {typeof(TInstance).FullName}.");
                return;
            }
        ...
        public static bool IsBlacklisted(string prefabName)
        {
            ...
            {
                string blacklistPath = Path.Combine(Main.modEntry.Path, "blacklist.txt");
                ...
                try
                {
                    BlacklistedItems = File.ReadAllLines(blacklistPath);
                }
                catch (Exception ex)
                {
                    Main.modEntry.Logger.Error($"Error reading blacklist: {ex.Message}");
                }
            }
            ...
            foreach (string item in BlacklistedItems)
            {
                ...
                if (prefabName.Equals(item.Trim(), StringComparison.OrdinalIgnoreCase))
                {
                    return true;
                }
            }
        ...
        public static bool IsWhitelisted(string prefabName)
        {
            if (WhitelistedItems.Length == 0)
            {
                string whitelistPath = Path.Combine(Main.modEntry.Path, "whitelist.txt");
                ...
                try
                {
                    WhitelistedItems = File.ReadAllLines(whitelistPath);
                    Main.modEntry.Logger.Log($"Loaded {WhitelistedItems.Length} items from whitelist.txt");
                }
                catch (Exception ex)
                {
                    Main.modEntry.Logger.Error($"Error reading whitelist: {ex.Message}");
                    return false;
                }
            }
    
            foreach (string item in WhitelistedItems)
            {
                if (string.IsNullOrWhiteSpace(item)) continue;
